...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:Mina0630@localhost/flask_api_db'
...
#    db.session.commit()
#    return jsonify({"message": f"User deleted successfully {id}"}), 200
...
def adopt_pet(user_id, pet_id):
    ...
    db.session.commit()
    return jsonify({"message": f"User {user.name} adopted the {pet.animal}, {pet.name}!"}), 200
...
def add_pets(user_id):
    ...
    for id in pet_data['pet_ids']:
        ...
        if pet is not None:
            ...
            return jsonify({"error": f"Pet with id {id} not found"}), 404
    db.session.commit()
    return jsonify({"message": "All pets adopted!"}), 200

# List all pets
@app.route("/pets", methods=["GET"])
def get_pets():
    pets = db.session.query(Pet).all()
    return pets_schema.jsonify(pets), 200
